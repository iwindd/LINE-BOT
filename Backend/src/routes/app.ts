...
import { ConfigKey } from '../typings/config';
const Route = express.Router()

Route.post("/set", async (req: Request, res: Response) => {
    if (!req.session.auth) return res.sendStatus(403)

    await SetConfig(req.session.auth, req.body.key, req.body.value)
    res.sendStatus(200)
});
...
Route.get("/get", async (req: Request, res: Response) => {
    ...
    const config = await LoadConfig(req.session.auth, req.query.key as ConfigKey)
